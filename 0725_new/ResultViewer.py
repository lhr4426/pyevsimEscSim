...
class ResultViewer(BehaviorModelExecutor) :
    ...
    def __init__(self, instance_time, destruct_time, name, engine_name, map_size, agent_count) :
        ...
        self.current_map = [['■'] * map_size for _ in range(map_size)]
        self.end_point = []
    ...
    def output(self):
        if self.get_cur_state() == "PrintMap" :
            ...
            for i in range(self.agent_count) :
                if self.agent_location[i] == self.end_point :
                    self.current_map[self.agent_location[i][0]][self.agent_location[i][1]] = '□'
                else :
                    self.current_map[self.agent_location[i][0]][self.agent_location[i][1]] = '■'
    ...
    def int_trans(self):
        ...
        elif self.get_cur_state() == "Move" and self.move_count > max([len(self.agent_move_log[i]) for i in range(self.agent_count)]):
